...
export default function Trade() {
  ...
  const handleTrade = async (type) => {
    ...
    try {
      ...
      if (res.ok) {        
        setMessage(`Trade successful! Executed at ${data.executed_price}`);
        setMessageType("success");
        setBalance(data.new_balance);
        fetchAll(); // Refresh balance and history
      } else {
        setMessage(data.detail || "Trade execution failed");
        setMessageType("error");
      }
    } catch (error) {
      console.error(error);
      setMessage("Connection error: Could not reach the execution server");
      setMessageType("error");
    }
  };
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {transactions.slice(0, 20).map((row, idx) => {
                const displayDate = row.timestamp ? new Date(row.timestamp).toLocaleString([], { 
                  dateStyle: 'short', 
                  timeStyle: 'short' 
                }) : "Pending";
