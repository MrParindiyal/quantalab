...
class StockAnalyzer:
    ...
    def train_prediction_model(self, data):
        ...
        feature_cols = [
            col for col in df.columns if not any(exc in col for exc in exclude_cols)
        ]
        feature_cols = [
            col
...
def create_advanced_chart(data, symbol):
    ...
    if all(col in data.columns for col in ["MACD", "MACD_signal", "MACD_histogram"]):
        ...
        histogram_colors = [
            "#00ff88" if val >= 0 else "#ff4444" for val in data["MACD_histogram"]
        ]
        fig.add_trace(
            go.Bar(
                x=data.index,
    ...
    if "RSI" in data.columns:
        ...
        # RSI levels
        fig.add_hline(
            y=70, line_dash="dash", line_color="red", opacity=0.7, row=4, col=1
        )
        fig.add_hline(
            y=30, line_dash="dash", line_color="green", opacity=0.7, row=4, col=1
        )
        fig.add_hline(
            y=50, line_dash="dot", line_color="gray", opacity=0.5, row=4, col=1
        )
...
def main():
    ...
    stock_choice = st.sidebar.selectbox(
        "🏢 Select Stock:", options=list(popular_stocks.keys()) + ["Custom"], index=0
    )

    if stock_choice == "Custom":
        symbol = st.sidebar.text_input(
            "Enter Stock Symbol:", value="AAPL", max_chars=10
        ).upper()
    else:
    ...
    period = st.sidebar.selectbox(
        "📅 Analysis Period:", options=["1mo", "3mo", "6mo", "1y", "2y", "5y"], index=3
    )
    ...
    with col2:
        ...
        avg_volume = data["Volume"].rolling(20).mean().iloc[-1]
        volume_change = (
            ((volume - avg_volume) / avg_volume) * 100 if avg_volume > 0 else 0
        )
        st.metric(
            label="📊 Volume",
            value=f"{volume:,.0f}",
    ...
    with col3:
        if "RSI" in data.columns and not pd.isna(data["RSI"].iloc[-1]):
            ...
            st.metric(label="⚡ RSI (14)", value=f"{rsi:.1f}", delta=rsi_status)
        else:
